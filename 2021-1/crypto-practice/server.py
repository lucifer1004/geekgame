...
def doRichard():
    print(MESenc(pad(("Hello, Alice! I will give you two flags. The first is: " +
                      flag1(token)).encode("utf-8")), Public_key).hex())
    print(MESenc(pad(("Sorry, I forget to verify your identity. Please give me your certificate.").encode(
        "utf-8")), Public_key).hex())
    ...
    comm_key = pow(int.from_bytes(akey, 'big'),
                   int.from_bytes(Richard_key, 'big'), P)
    for i in range(128*8):
        if (comm_key >> i) & 1:
            old_key = (old_key ^ (old_key << (i+1))) % (2**2048)
    new_key = int.to_bytes(old_key, 256, 'big')
    print(MESenc(pad(("I can give you the second flag now. It is: " +
                      flag2(token)).encode("utf-8")), new_key).hex())
    return
...
Richard_cert = register(b"Richard", pow(
    G, int.from_bytes(Richard_key, 'big'), P).to_bytes(128, 'big'))
Alice_key = os.urandom(128)
Alice_cert = register(b"Alice", pow(G, int.from_bytes(
    Alice_key, 'big'), P).to_bytes(128, 'big'))
print(Alice_cert)
